...
def const_accel(
    ...
    assert np.abs(pow(vf, 2) - pow(vi, 2)) / 2 < a_max * (
        df - di
    ), "Error 1: Trajectory is not possible given inputs."
    assert (
        np.sqrt((df - di) / a_max) * 2 > 1
    ), "Error 2: Trapezoidal trajectory is not possible given inputs."

    # If the below condition is true, then the target velocity is met

    if (df - di) * a_max > (pow(v_target, 2) - ((pow(vi, 2) + pow(vf, 2)) / 2)):
        Ta = np.abs(v_target - vi) / a_max  # Use abs if vi is greater than v_target
...
def _accel_q(
    di: float, vi: float, ti: float, v_target: float, Ta: float, t: float
) -> float:
    return di + (vi * (t - ti)) + (((v_target - vi) / (2 * Ta)) * pow((t - ti), 2))


def _accel_v(vi: float, v_target: float, Ta: float, t: float, ti: float) -> float:
    return vi + ((v_target - vi) / Ta) * (t - ti)


def _const_q(
    di: float, vi: float, Ta: float, v_target: float, ti: float, t: float
) -> float:
    return di + (vi * (Ta / 2)) + (v_target * (t - ti - (Ta / 2)))


def _decel_q(
    df: float, vf: float, T: float, t: float, v_target: float, Td: float
) -> float:
    return df - (vf * (T - t)) - (((v_target - vf) / (2 * Td)) * pow((T - t), 2))


def _decel_v(vf: float, v_target: float, Td: float, T: float, t: float) -> float:
